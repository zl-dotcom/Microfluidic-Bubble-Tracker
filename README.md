...
* **⏱️ 交互式进度与倍速控制：** 支持自由拖动进度条跳转视频时间，并可通过滑块实时调整播放速度。支持极速快进，或最高约 10 倍速的慢动作回放，方便您肉眼仔细核对极速流过的液滴。
* **⚠️ 严谨的算法预热机制：** 为了建立准确无误的背景消除模型，系统在每次开始运行或您**每次拖动进度条跳转后**，都会强制进行 **0.5 秒的算法预热**。请注意：在这 0.5 秒的预热期内，算法处于背景学习状态，流过的液滴将不会被计数。
